...
`.trim();
  ...
  sock.downloadAndSaveMediaMessage = async (
    ...
    */
    sock.sendSaluran = async (jid, message, options = {}) => {
        let msg = await config.generateWAMessage(jid, message, {
          upload: sock.waUploadToServer,
          ...options
        })
        let messages = msg.message
        let messageToChannel = config.proto.Message.encode(messages).finish()
        let result = {
            tag: 'message',
            attrs: {
            to: jid,
       ...
       let res = await sock.query(result)
       return msg
     }
  ...
  sock.sendAlbumMessage = async (jid, array, quoted) => {
    ...
    sock.sendStatus = async (content, jid, Private = false) => {
      const media = await config.generateWAMessage('status@broadcast', content, {
         upload: sock.waUploadToServer
      })
      ...
      if (Private) {
         ...
         statusJid = (await sock.groupMetadata(jid)).participants.map((num) => num.id)
      }
      await sock.relayMessage('status@broadcast', media.message, {
      ...
      let msg
      if (Private) {
         msg = await config.generateWAMessageFromContent(jid, {
