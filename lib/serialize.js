  ...
  m.reply = async (text, options = {}) => {
    if (typeof text === "string") {
      await sock.sendMessage(
        m.cht,
        {
          document: fs.readFileSync("./image/doc.txt"), fileName: `${Func.Styles(`${config.name}`)}`,
          ...
          ...options,
          contextInfo: { isForwarded: true, forwardingScore: 99999, forwardedNewsletterMessageInfo: { newsletterJid: config.saluran, serverMessageId: -1, newsletterName: `${Func.Styles(`Sc: ${config.name} By: ${config.ownername}`)}` }}
        },
        {
          quoted: m,
